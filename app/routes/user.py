...
def dashboard():
    ...
    return render_template('user_home.html', user=user, history=history, parking_lots=parking_lots, search_location=None)
...
def reserve_spot():
    ...
    try:
        ...
        db.session.rollback()
        flash(f'Failed to reserve spot: {str(e)}', 'danger')
